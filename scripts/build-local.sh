...
TEMP_DIR="/tmp/salesforce-cloud-mcp"
mkdir -p "$TEMP_DIR"
...
if [ "$VERBOSE" = true ]; then
    if docker build -t salesforce-cloud:local .; then
    ...
    if docker build -t salesforce-cloud:local . > "$DOCKER_LOG" 2>&1; then
...
echo -e "\n${GREEN}Build complete!${NC} Image tagged as salesforce-cloud:local"
